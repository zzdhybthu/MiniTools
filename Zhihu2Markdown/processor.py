...
class Processor:
	...
	def _process_markdown(self):
		...
		pattern = r'\\bm'
		self.markdown = re.sub(pattern, r'\\boldsymbol', self.markdown)
  
		pattern = r'\\left\\\\'
		self.markdown = re.sub(pattern, r'\\left\\', self.markdown)
  
		pattern = r'\\right\\\\'
		self.markdown = re.sub(pattern, r'\\right\\', self.markdown)
