...
def get_play_list(aid, cid, quality):
    ...
    headers = {
        ...
        'Cookie': '00948a26%2C1724251506%2C80bb1%2A21CjDpl_tclMy6f_V25HiZ160Bj_d9GDw9CAegARpLOGWUWwyd9MbfcU031lQDl9KJDjsSVlU2R2JTeXBrZEp3b2I1cHVtUlJzWldXczNwNUdsYlFQdGRXb05PV2YtekF1MkFMQ3U0ek9WQ25uVGI1STZ2QzMyMkk4YWNzT1JXaC1DQTJjV2lqdnNBIIEC; bili_jct=8c8a8df2a0bef423dd07da6bdb78854b', # 登录B站后复制一下cookie中的SESSDATA字段,有效期1个月
...
def combine_video(video_list, title):
    currentVideoPath = '/Users/zhenzhengdehuoyubai/Desktop/Tools/DownloadVideos'  # 自定义下载目录
    if len(video_list) >= 2:
        # 视频大于一段才要合并
...
def default_main():
    ...
    if start.isdigit() == True:
        ...
        # 获取cid的api, 传入aid即可
        aid = start
        start_url = 'https://api.bilibili.com/x/web-interface/view?bvid=' + aid
    else:
        # 如果输入的是url (eg: https://www.bilibili.com/video/av46958874/)
        aid = re.search(r'/video/([0-9a-zA-Z]+)/*', start).group(1)
        start_url = 'https://api.bilibili.com/x/web-interface/view?bvid=' + aid
    # qn参数就是视频清晰度
    # 可选值：
