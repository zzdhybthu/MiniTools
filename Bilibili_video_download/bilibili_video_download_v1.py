...
def down_video(video_list, title, start_url, page):
    ...
    for i in video_list:
        ...
        # 开始下载
        if len(video_list) > 1:
            urllib.request.urlretrieve(url=i, filename=os.path.join(currentVideoPath, r'{}-{}.mp4'.format(title, num)),reporthook=Schedule_cmd)  # 写成flv也行  title + '-' + num + '.flv'
        else:
            urllib.request.urlretrieve(url=i, filename=os.path.join(currentVideoPath, r'{}.mp4'.format(title)),reporthook=Schedule_cmd)  # 写成flv也行  title + '-' + num + '.flv'
        num += 1
...
def main():
    ...
    if start.isdigit() == True:  # 如果输入的是av号
        ...
        start_url = 'https://api.bilibili.com/x/web-interface/view?bvid=' + re.search(r'/video/([0-9a-zA-Z]+)/*', start).group(1)
